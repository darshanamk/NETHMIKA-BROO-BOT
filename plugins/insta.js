...
const LOAD_ING = "```Downloading media for SL-BROO-BOT...```"
...
SL-BROO-BOT.addCommand({ pattern: 'insta ?(.*)', fromMe: false, desc: "Downloads from instagaram🙈", dontAddCommandList: true }, async (message, match) => { 
    const { status, type, data } = await instagram(match[1], 'SL-BROO-BOT')
    if (!status) return await message.sendMessage('not found')
    await message.client.sendMessage(message.jid, LOAD_ING, MessageType.text, { quoted: message.data });
    if (type === 'image') return await message.sendMessage(data, MessageType.image, { caption: "𝙼𝚊𝚍𝚎 𝙱𝚢 𝙽𝚎𝚝𝚑𝚖𝚒𝚔𝚊 𝙱𝚛𝚘𝚘🙈", quoted: message.data })
    if (type === 'video') return await message.sendMessage(data, MessageType.video, { caption: "𝙼𝚊𝚍𝚎 𝙱𝚢 𝙽𝚎𝚝𝚑𝚖𝚒𝚔𝚊 𝙱𝚛𝚘𝚘🙈", quoted: message.data })
});
